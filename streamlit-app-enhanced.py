...
@st.cache_resource
def cargar_modelo():
    """Carga el modelo guardado con manejo de errores mejorado"""
    try:
        model_files = [f for f in os.listdir('models') if f.endswith('.joblib')]
        if not model_files:
            raise FileNotFoundError("No se encontraron archivos .joblib en la carpeta models")
        ...
        st.error("❌ No se encontró el archivo del modelo. Verifique que existe la carpeta 'models' con el modelo entrenado.")
        return None
    except Exception as e:
        logging.error(f"Error inesperado al cargar el modelo: {str(e)}")
...
def crear_campos_formulario():
    """Crea los campos del formulario de entrada con validaciones mejoradas"""
...
def mostrar_resultado(prediccion, datos, probabilidad=None):
    """Muestra el resultado de la predicción con métricas adicionales"""
